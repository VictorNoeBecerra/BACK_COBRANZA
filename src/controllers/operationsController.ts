...
// console.log( moment().subtract(6,'h'));
  ...
  public async getResumeCobros(req: Request, res: Response): Promise<void> {
    ...
    SUM(o.costos) AS costos
        FROM
        operaciones o WHERE DATE(o.date) BETWEEN '${moment(day?.toString()).startOf('week').format('YYYY-MM-DD')}' AND '${moment(day?.toString()).add(1, 'days').format('YYYY-MM-DD')}' GROUP BY o.date, o.repartidor;`
